...
**7. IAM stores the email with the casing Google has on file, not the casing you typed.** The binding was created as `user:someone@example.com` and stored as `user:SomeOne@example.com`, matching the casing on the Google account rather than the casing typed. Removing it with the lowercase form fails with "Policy binding with the specified principal, role, and condition not found", which reads like the binding was never made. Read the policy back before removing a binding by hand.
